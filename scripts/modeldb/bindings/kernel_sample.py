...
def main():
    ...
    kernel_baseline = build_kernel(Ns, d, M, C) 
    kernel_test = build_paged_kernel(Ns, d, M, C)
    ...
    out_base = kernel_baseline(query, key_codes, value_codes, key_cents, value_cents,
                    key_residuals, value_residuals, r, partial_out, partial_lse)
    out_test = kernel_test(query, key_codes, value_codes, key_cents, value_cents,
                    key_residuals, value_residuals, r, partial_out, partial_lse)
    ...
    print("Evaluate Baseline Kernel")
    mae = (out_base - out_ref).abs().mean().item()
    mxe = (out_base - out_ref).abs().max().item()
    print(f"shape: {tuple(out_base.shape)}, MAE: {mae:.4e}, MaxAbsErr: {mxe:.4e}")
    # 性能：预热 + 多次计时
    for _ in range(10):
        kernel_baseline(query, key_codes, value_codes, key_cents, value_cents,
                key_residuals, value_residuals, r, partial_out, partial_lse)
    ...
    print(f"kernel_Baseline: avg time per call: {avg_ms:.3f} ms  (Ns={Ns}, d={d}, M={M}, C={C}, T={T}, r={r})")
    
    
    print("Evaluate Test Kernel")
    mae = (out_test - out_ref).abs().mean().item()
    mxe = (out_test - out_ref).abs().max().item()
    print(f"shape: {tuple(out_test.shape)}, MAE: {mae:.4e}, MaxAbsErr: {mxe:.4e}")
    # 性能：预热 + 多次计时
    for _ in range(10):
        kernel_test(query, key_codes, value_codes, key_cents, value_cents,
                key_residuals, value_residuals, r, partial_out, partial_lse)
    torch.cuda.synchronize()
    t0 = time.time()
    for _ in range(args.iters):
    ...
    torch.cuda.synchronize()
    avg_ms = (time.time() - t0) / args.iters * 1e3
    print(f"kernel_test: avg time per call: {avg_ms:.3f} ms  (Ns={Ns}, d={d}, M={M}, C={C}, T={T}, r={r})")
