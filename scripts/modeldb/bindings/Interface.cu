...
(
	...
	const torch::Tensor key_cents, // shaped (M, C, d/M)
	const torch::Tensor key_residuals, // shaped (bs, nh_k, Lt, d)
	const torch::Tensor value_page_ids, // shaped (bs, nh_k, n_pages) - int64
	const torch::Tensor value_page_pool, // shaped (max_pages, M, page_size) - transposed storage
	const torch::Tensor value_cents, // shaped (M, C, d/M)
	const torch::Tensor value_residuals, // shaped (bs, nh_k, Lt, d)
	const int r, // residual length, 0 < r <= Lt
...
)
{
	...
	// compute ad_lut for key_codes, shaped (bs, nh, M, 1, C). 
	auto query_reshaped = query.reshape({bs, nh, 1, M, d/M}).transpose(2, 3);
	auto ad_lut = at::matmul(query_reshaped, key_cents.transpose(1, 2));
	query_reshaped = torch::Tensor(); // release memory
	...
	// Fill partial_out[:, :, :Ns] and partial_lse[:, :, :Ns] using paged V storage
	{
		dim3 split_grid(bs, nh, Ns);
		dim3 split_block(Lt, 1, 1);
		
		flash_decoding_paged_v_kernel<cuscalar_t, code_t, Ns, Lt, d, M, C><<<split_grid, split_block>>>(
			reinterpret_cast<const cuscalar_t*>(ad_lut.data_ptr<scalar_t>()),
			key_codes.data_ptr<code_t>(),
			value_page_ids.data_ptr<int64_t>(),
			value_page_pool.data_ptr<code_t>(),
			reinterpret_cast<const cuscalar_t*>(value_cents.data_ptr<scalar_t>()),
			partial_out,
			partial_lse,
			bs,
	...
	// Fill partial_out[:, :, -1] and partial_lse[:, :, -1] using residual cache
	{
		dim3 residual_grid(bs, nh);
		dim3 residual_block(d, 1, 1);
...
#define register_flash_decoding_paged_v(T, code_alias, Nsv, Ltv, dv, Mv, Cv) \
	torch::Tensor flash_decoding_paged_v_##T##code_alias##_Ns##Nsv##Lt##Ltv##d##dv##M##Mv##C##Cv( \
		...
		return flash_decoding_paged_v<T, code_alias, Nsv, Ltv, dv, Mv, Cv>( \
			query, \
			key_codes, \
			key_cents, \
			key_residuals, \
